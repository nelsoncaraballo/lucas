...
class MSound extends MObject {
  static const String TableName = 'sounds';
  String localSoundFileName = ''; // local filename of this audio

  static List<MSound> inMemoryTable;
  static Map<int, MSound> inMemoryDictionary;
  ...
  Future<MSound> clone() async {
    ...
    newObject.localSoundFileName = this.localSoundFileName;
  ...
  factory MSound.fromMap(Map<String, dynamic> json) => new MSound(
        id: json["id"],
        ...
        localSoundFileName: json["localSoundFileName"],
  ...
  Map<String, dynamic> toMap() => {
        ...
        "localSoundFileName": localSoundFileName,
  ...
  static populateInMemoryTables() async {
    if (inMemoryTable == null) {
      ...
      String sql = 'SELECT $TableName.*, '
          ...
          'AND ${Translation.TableName}.tableName="${MSound.TableName}" '
      ...
      List<MSound> list = result.isNotEmpty
          ? result.toList().map((c) => MSound.fromMap(c)).toList()
          : <MSound>[];
  ...
  static Future<void> createWithID(MSound entity) async {
    final db = await DBProvider.db.database;
    // var res = await db.insert("$TableName", entity.toMap());
    // return res;

    await db.rawInsert(
        "INSERT Into $TableName (id,fileName,categoryId, isVisible, isUnderstood, useAsset, localFileName, userCreated, localSoundFileName,isAvailable,user,backgroundColor)"
        " VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        [
          entity.id,
          entity.fileName,
  ...
  static Future<List<MSound>> getAll({
    String textToSearch = '',
  }) async {
  ...
  static Future<List<MSound>> getUserCreated(String user) async {
    await populateInMemoryTables();
  ...
  static Future<MSound> getByID(int id) async {
    await populateInMemoryTables();

    return MSound.inMemoryDictionary[id];
    // String languageCode =
    //     await LocalPreferences.getString('languageCode', 'en');
    ...
    // if (results.length > 0) {
    //   MSound result = MSound.fromMap(results.first);
  ...
  static Future<Map<int, MSound>> getSeveralByID(List<int> ids) async {
    Map<int, MSound> result = new Map();
    for (int id in ids) {
      MSound mSound = await MSound.getByID(id);
      result[id] = mSound;
    }
  ...
  static Future<void> update(MSound entity) async {
    final db = await DBProvider.db.database;
  ...
  static Future<void> populateFromJson(bool replaceExistingInfo) async {
    String assetName = 'assets/catalogs/sounds.json';
  ...
  static Future<void> saveAssetInDatabase(
    ...
    for (MapEntry<String, dynamic> item in jsonMap.entries) {
      ...
      batch.rawInsert(
          "INSERT Into $TableName (id,fileName,categoryId, isVisible, isUnderstood, useAsset, localFileName, userCreated, localSoundFileName, isAvailable, user) "
          "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
          [
            entity.id,
            entity.fileName,
    ...
    //   await Translation.populateFromJson(replaceExistingInfo, MSound.TableName,
    //       'sounds', language.languageCode);
    // }
  }

  static Future<void> populateFromJsonImport() async {
    String assetName = 'assets/catalogs/sounds_import.json';
    ...
    for (MapEntry<String, dynamic> item in jsonMap.entries) {
      ...
      String localSoundFileName = item.value["localSoundFileName"];
      ...
      batch.rawInsert(
          "INSERT Into $TableName (id,fileName,isVisible,isUnderstood,useAsset,localFileName,userCreated,localSoundFileName,isAvailable,minLevelToShow,backgroundColor,user) "
          "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
          [
            entity.id,
            entity.fileName,
  ...
  static Future<void> loadFromAssets() async {
    ...
    await MSound.createTableIFNotExists();
    // await Translation.deleteForTable(MSound.TableName);
    await MSound.deleteAll();
  ...
  static updateBackgroundColor() async {
    List<MSound> objects = await getAll();
  ...
  static Future<WebResponse> backupUserCreatedObjectsContent() async {
    ...
    for (int i = 0; i < objects.length; i++) {
      MSound obj = objects[i];
      //if (obj.userCreated == 1 && obj.useAsset == 0) {
      if (obj.localSoundFileName.length > 0) {
        File f = File('${Helper.appDirectory}/${obj.localSoundFileName}');
        //Image.file(f);
        final bytes = await f.readAsBytes();
        if (bytes != null) {
          String base64 = Base64Encoder().convert(bytes);
          ...
          String json = jsonEncode({
            ...
            "operation": "upload asset",
            "objectType": 'image',
            "fileName": obj.localSoundFileName,
            "id": obj.id,
            "propertyName": 'localSoundFileName',
            "base64": base64,
          });
          webResponse = await Helper.invokeWebService(json);
          if (!webResponse.operation) {
